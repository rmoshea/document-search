...
if user_search_file == "1":
	content = filename1
	print "\nYou have selected the file: french_armed_forces.txt"
elif user_search_file == "2":
	content = filename2
	print "\nYou have selected the file: hitchhickers.txt"
elif user_search_file == "3":
	content = filename3
	print "\nYou have selected the file: warp_drive.txt"
else:
	content = filename
...
#If user selects Regular Expressions as their search method --- "re.M" to signify multiline; re.I" used to ignore case
		...
		f = open(filename, 'r')
		words = sum(1 for w in f if re.search(user_search_value, w, re.M|re.I))
		f.close()
